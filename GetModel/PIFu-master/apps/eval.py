...
if __name__ == '__main__':
    wait_layout = [
        [psg.Text('建模中請稍等一兩分鐘...')],
        [psg.ProgressBar(100, orientation='h', size=(20, 20), key='wait_bar')],
        [psg.Text('', key='wait_key')]
    ]
    wait_window = psg.Window('正在處理中', wait_layout, finalize=True, modal=True, no_titlebar=True)
    for i in range(0, 11):
        time.sleep(0.05)
        wait_window['wait_key'].update(f'{i}%')
        wait_window['wait_bar'].update(i)
